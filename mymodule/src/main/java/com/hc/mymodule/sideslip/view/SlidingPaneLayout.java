...
package com.hc.mymodule.sideslip.view;
...
//import com.hengchang.activity.common.utils.LogUtils;
...
public class SlidingPaneLayout extends ViewGroup {
	...
	public SlidingPaneLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mOverhangSize = 0/* (int) (DEFAULT_OVERHANG_SIZE * density + 0.5f) */;// hack
																				// by
																				// Fooyou
...
//					LogUtils.e(TAG, "drawChild: child view " + child + " returned null drawing cache");
					result = super.drawChild(canvas, child, drawingTime);
				}
			} else {
				if (child.isDrawingCacheEnabled()) {
	...
	private boolean isIgnoredViewUnder(MotionEvent ev) {
		for (View v : mIgnoredViews) {
			if (v != null && isPointInsideView((int) ev.getX(), (int) ev.getY(), v)) {
				return true;
			}
		}
		return false;
	...
	private void findIgnoredView(View v) {
		/* if(BuildConfig.DEBUG)LogUtils.d(TAG, "findIgnoredView"); */
		if (v instanceof ViewGroup) {
